...
//
// Write to the console any existing conflict resolution issues
//
FlatWriter.prototype.describeConflictResolutions = function() {
  if ( this.quiet ) {
    return;
  }

  console.log( "ckupdate conflict resolutions:" );

  var packages = this.packages;
  var resolutionsFound = false;
    ...
    if ( packages.hasOwnProperty( key ) ) {
      ...
      if ( packageArray.length > 1 ) {
        ...
        for ( var i = 0; i < packageArray.length; i++ ) {
          var isPrimary = packageArray[ i].isPrimaryVersion;
          versions += ( i == 0 ) ? ( isPrimary ? "*" : "" ) : ", " + ( isPrimary ? "*" : "" );
          versions += packageArray[ i ].packageVersion + ( isPrimary ? "*" : "" );
        }
        console.log( key + VERSION_DELIM + ": " + versions );
...
FlatWriter.prototype.write = function() {
  ...
  if ( !quiet ) {
    console.log( "ckupdate flattening..." );
...
function writePackageItem( packageItem, params ) {
  ...
  if ( !params.quiet ) {
    console.log( "├── " + packageItem.packageName + VERSION_DELIM + packageItem.packageVersion );
  }
