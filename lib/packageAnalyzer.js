...
PackageAnalyzer.prototype.loadPackageJson = function( path ) {
  ...
  .catch( function( error ) {
    // ENOENT errors are expected for directories not containing a package.json file
...
// isRecursive: If true, recurse into each package directory
//
PackageAnalyzer.prototype.analyzeNodeModulesPath = function( nodeModulesPath, isRecursive ) {
  ...
  .catch( function( error ) {
    // ENOENT errors are expected for directories not containing a node_modules directory
    if ( error.cause && error.cause.code != "ENOENT" ) {
...
PackageAnalyzer.prototype.describeConflictResolutions = function() {
  for ( var key in packages ) {
    if ( packages.hasOwnProperty( key ) ) {
      var packageArray = packages[ key ];
      if ( packageArray.length > 1 ) {
        ...
        for ( var i = 0; i < packageArray.length; i++ ) {
          versions += ( i == 0 ) ? "#" : ", #";
          versions += packageArray[ i ].packageVersion;
        }
...
function hasFlattenKeyword( json ) {
  ...
  for ( var i = 0; i < json.keywords.length; i++ ) {
    if ( json.keywords[ i ] === FLATTEN_KEYWORD ) {
      return true;
    }
  }
